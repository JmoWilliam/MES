...
{
    class RptMoInformationSynchronize
    {
        ...
        public RptMoInformationSynchronize(string company, string secretKey)
        {
        ...
        public void Init()
        {
            try
            {
                string targetDate = targetDate = DateTime.Now.AddDays(-30).ToString("yyyy-MM-dd HH:mm:ss"),
                    targetUrl = string.Format("{0}{1}", domainUrl, "api/MES/RptMoInformationNotifier");

                var postData = new List<KeyValuePair<string, string>>
                {
                    new KeyValuePair<string, string>("Company", company),
                    new KeyValuePair<string, string>("SecretKey", secretKey),
                    new KeyValuePair<string, string>("ActualStart", targetDate)
                };

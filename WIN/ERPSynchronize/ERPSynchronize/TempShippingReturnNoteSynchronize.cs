...
{
    class TempShippingReturnNoteSynchronize
    {
        ...
        public TempShippingReturnNoteSynchronize(string company, string secretKey)
        {
        ...
        public void Init()
        {
            try
            {
                string targetDate = DateTime.Now.AddDays(-2).ToString("yyyyMMdd HH:mm:ss"),
                    targetUrl = string.Format("{0}{1}", domainUrl, "api/ERP/TempShippingReturnNoteSynchronize");

...
{
    internal class TemporaryOrderSynchronize
    {
        ...
        public TemporaryOrderSynchronize(string company, string secretKey)
        {
        ...
        public void Init()
        {
            try
            {
                string targetDate = DateTime.Now.AddDays(-2).ToString("yyyyMMdd HH:mm:ss"),
                    targetUrl = string.Format("{0}{1}", domainUrl, "api/ERP/TemporaryOrderSynchronize");
